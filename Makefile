...
.PHONY: generate
generate: $(VGOPATH) $(HELM) $(YQ) generate-charts
...
generate-charts: $(VGOPATH) $(HELM) $(YQ)
	@REPO_ROOT=$(REPO_ROOT) VGOPATH=$(VGOPATH) GARDENER_HACK_DIR=$(GARDENER_HACK_DIR) bash $(GARDENER_HACK_DIR)/generate-sequential.sh ./charts/...

.PHONY: generate-in-docker
generate-in-docker: tidy install $(HELM) $(YQ)
	docker run --rm -i$(DOCKER_TTY_ARG) -v $(PWD):/go/src/github.com/metal-stack/gardener-extension-csi-driver-synology golang:$(GO_VERSION) \
		sh -c "cd /go/src/github.com/metal-stack/gardener-extension-csi-driver-synology \
				&& make generate \
...
push-to-gardener-local:
	CGO_ENABLED=1 go build \
		...
		-o bin/gardener-extension-csi-driver-synology \
		./cmd/gardener-extension-csi-driver-synology
	docker build -f Dockerfile.dev -t ghcr.io/metal-stack/gardener-extension-csi-driver-synology:latest .
	kind --name gardener-local load docker-image ghcr.io/metal-stack/gardener-extension-csi-driver-synology:latest
